<script>
	...
	const icons = {
		...
		article:
			'M12 7.5h1.5m-1.5 3h1.5m-7.5 3h7.5m-7.5 3h7.5m3-9h3.375c.621 0 1.125.504 1.125 1.125V18a2.25 2.25 0 0 1-2.25 2.25M16.5 7.5V4.875c0-.621-.504-1.125-1.125-1.125H4.125C3.504 3.75 3 4.254 3 4.875V18a2.25 2.25 0 0 0 2.25 2.25h13.5M6 7.5h3v3H6v-3Z',
...
<!-- Mobile sidebar -->
<div
	class="fixed inset-y-0 left-0 z-50 lg:hidden {mobileOpen
		? 'translate-x-0'
		: '-translate-x-full'} transition-transform duration-300 ease-in-out"
>
	<div
		class="relative flex h-full w-72 flex-col border-r border-white/10 bg-gray-900/80 shadow-2xl shadow-black/50 backdrop-blur-2xl"
	>
		...
		<!-- Subtle glow effect -->
		<div
			class="absolute top-0 -right-px bottom-0 w-px bg-gradient-to-b from-blue-500/20 via-purple-500/10 to-blue-500/20"
		></div>
		<!-- Header -->
		<div
			class="relative z-10 flex h-16 items-center justify-center border-b border-white/10 bg-white/[0.02] px-4"
		>
			...
			<button
				on:click={closeMobile}
				class="absolute right-4 rounded-lg p-2 text-gray-400 transition-colors hover:bg-white/10 hover:text-white"
		...
		<nav class="relative z-10 flex-1 overflow-y-auto px-3 py-4">
			...
			{#if user}
				<div class="mb-6">
					<p class="mb-2 px-3 text-xs font-medium tracking-wider text-gray-500 uppercase">My AGE</p>
					<ul class="space-y-1">
						{#each personalNavItems as item}
							...
							{/if}
						{/each}
						<!-- Staff Events link (for users assigned to events) -->
						{#if assignedEventsCount > 0}
							<li>
								...
								>
									...
									<span>My Events</span>
									<span
										class="ml-auto rounded-full bg-blue-500/20 px-2 py-0.5 text-xs text-blue-400"
										>{assignedEventsCount}</span
									>
								</a>
							</li>
						{/if}
		...
		{#if !user || (user.role !== 'premium' && user.role !== 'admin')}
			<div class="relative z-10 mx-3 mb-3">
				<div
					class="relative overflow-hidden rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-900/30 via-gray-900 to-purple-900/20 p-4"
				>
					<!-- Decorative glow -->
					<div
						class="absolute -top-4 -right-4 h-24 w-24 rounded-full bg-emerald-500/20 blur-2xl"
					></div>
					<div class="relative">
						...
						<a
							href="/premium"
							on:click={closeMobile}
							class="block w-full rounded-lg bg-gradient-to-r from-emerald-600 to-green-700 px-4 py-2 text-center text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all hover:from-emerald-500 hover:to-green-600"
						>
							Join Premium
						</a>
					</div>
		...
		<!-- User Section (Bottom) -->
		<div class="relative z-10 border-t border-white/10 bg-white/[0.02] p-4">
			{#if user}
				{@const isPremiumUser = user.role === 'premium' || user.role === 'admin'}
				<div class="flex items-center gap-3">
					<div class="relative">
						<div
							class="flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold text-white shadow-lg transition-all duration-200
							{isPremiumUser
								? 'bg-gradient-to-br from-emerald-400 via-green-500 to-emerald-600 ring-2 shadow-emerald-500/40 ring-emerald-400/50'
								: 'bg-gradient-to-br from-blue-500 to-purple-600 ring-2 shadow-blue-500/25 ring-white/10'}"
						>
							{getUserInitials(user.firstName, user.lastName)}
						</div>
						{#if isPremiumUser}
							<div
								class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-gradient-to-br from-emerald-400 to-green-600 shadow-lg shadow-emerald-500/50"
							>
					...
					<div class="min-w-0 flex-1">
						...
						{#if isPremiumUser}
							<span
								class="mt-1 inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-gradient-to-r from-emerald-500/20 to-green-600/20 px-2 py-0.5 text-[10px] font-semibold text-emerald-400"
							>
			...
			{:else}
				<div class="flex gap-2">
					<a
						href="/login"
						on:click={closeMobile}
						class="flex-1 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-center text-sm font-medium text-white backdrop-blur-sm transition-all duration-200 hover:border-white/20 hover:bg-white/10"
					>
						Login
					</a>
					<a
						href="/signup"
						on:click={closeMobile}
						class="flex-1 rounded-lg border border-blue-400/20 bg-gradient-to-r from-blue-500/80 to-purple-500/80 px-3 py-2 text-center text-sm font-medium text-white shadow-lg shadow-blue-500/20 backdrop-blur-sm transition-all duration-200 hover:border-blue-400/40 hover:from-blue-500 hover:to-purple-500"
					>
						Sign Up
					</a>
				</div>
...
>
	<div
		class="relative flex h-full flex-col border-r border-white/10 bg-gray-900/80 shadow-2xl shadow-black/50 backdrop-blur-2xl"
	>
		<!-- Glassmorphic gradient overlay -->
		<div
			class="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/5 via-transparent to-black/20"
		...
		<!-- Header -->
		<div
			class="relative z-10 flex h-16 items-center {collapsed
				? 'justify-center'
				: 'justify-between'} border-b border-white/10 bg-white/[0.02] px-4"
		>
			<a href="/" class="flex items-center">
				{#if collapsed}
					<!-- Icon only when collapsed -->
					<div
						class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg border border-gray-700 bg-gray-800"
					>
		...
		{#if collapsed}
			<div class="relative z-10 flex justify-center border-b border-white/10 py-3">
				<button
					on:click={toggleCollapse}
					class="rounded-lg p-2 text-gray-400 transition-colors hover:bg-white/10 hover:text-white"
		...
		<nav class="relative z-10 flex-1 overflow-y-auto {collapsed ? 'px-2' : 'px-3'} py-4">
			...
			{#if user}
				<div class="mb-6">
					{#if !collapsed}
						<p class="mb-2 px-3 text-xs font-medium tracking-wider text-gray-500 uppercase">
							My AGE
						</p>
					{/if}
					<ul class="space-y-1">
		...
		{#if !user || (user.role !== 'premium' && user.role !== 'admin')}
			...
			{:else}
				<div class="relative z-10 mb-3 flex justify-center">
					<a
						href="/premium"
						class="rounded-lg border border-emerald-500/30 bg-gradient-to-br from-emerald-500/20 to-green-500/20 p-2.5 text-emerald-400 transition-all hover:from-emerald-500/40 hover:to-green-500/40 hover:text-white"
						title="Join Premium"
		...
		<div class="relative z-10 border-t border-white/10 {collapsed ? 'p-2' : 'p-4'} bg-white/[0.02]">
					...
					<div class="flex flex-col items-center gap-2">
						<a href="/account" class="relative" title="Account">
							<div
								class="flex h-10 w-10 items-center justify-center rounded-full text-sm font-semibold text-white shadow-lg transition-all duration-200
								{isPremiumUser
									? 'bg-gradient-to-br from-emerald-400 via-green-500 to-emerald-600 ring-2 shadow-emerald-500/40 ring-emerald-400/50 hover:ring-emerald-300/70'
									: 'bg-gradient-to-br from-blue-500 to-purple-600 ring-2 shadow-blue-500/25 ring-white/10 hover:ring-blue-400/50'}"
							>
								{getUserInitials(user.firstName, user.lastName)}
							</div>
							{#if isPremiumUser}
			...
			{:else}
				<div class="flex flex-col gap-2">
					<a
						href="/login"
						class="w-full rounded-lg border border-white/10 bg-white/5 px-4 py-2.5 text-center text-sm font-medium text-white backdrop-blur-sm transition-all duration-200 hover:border-white/20 hover:bg-white/10 hover:shadow-lg hover:shadow-white/5"
					>
						Login
					</a>
					<a
						href="/signup"
						class="w-full rounded-lg border border-blue-400/20 bg-gradient-to-r from-blue-500/80 to-purple-500/80 px-4 py-2.5 text-center text-sm font-medium text-white shadow-lg shadow-blue-500/20 backdrop-blur-sm transition-all duration-200 hover:border-blue-400/40 hover:from-blue-500 hover:to-purple-500"
					>
						Sign Up
					</a>
				</div>
...
<div class="{collapsed ? 'lg:pl-16' : 'lg:pl-60'} transition-all duration-300 ease-in-out">
	<!-- Top bar for mobile -->
	<div
		class="relative sticky top-0 z-30 flex h-16 items-center justify-center border-b border-white/10 bg-gray-950/80 px-4 backdrop-blur-2xl lg:hidden"
	>
		<!-- Subtle gradient overlay -->
		<div
			class="pointer-events-none absolute inset-0 bg-gradient-to-r from-white/[0.02] to-transparent"
		></div>
		<button
			on:click={toggleMobile}
			class="absolute left-4 z-10 rounded-lg p-2 text-gray-400 transition-colors hover:bg-white/10 hover:text-white"
