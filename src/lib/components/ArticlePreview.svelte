...
{#if variant === 'featured'}
	...
	<a href="/articles/{article.slug}" class="group block">
		<article class="h-full">
			...
			{/if}
			{#if article.tags?.[0]}
				<span class="text-[10px] font-semibold uppercase tracking-wider text-blue-400">
					{article.tags[0].name}
				</span>
			{/if}
			<h3 class="mt-1 font-display text-xl font-bold leading-tight text-white transition-colors group-hover:text-blue-400 sm:text-2xl">
				{article.title}
			</h3>
			{#if article.excerpt}
			...
			<div class="mt-3 flex items-center gap-2 text-xs text-gray-500">
				...
				{#if article.isPremium}
					<span>·</span>
					<span class="inline-flex items-center gap-1 rounded bg-emerald-600 px-1.5 py-0.5 text-[10px] font-semibold text-white">
				...
				{:else if article.accessMode === 'Premium' || article.accessMode === 'premium'}
					<span>·</span>
					<span class="inline-flex items-center gap-1 rounded bg-gradient-to-r from-violet-600 to-fuchsia-600 px-1.5 py-0.5 text-[10px] font-semibold text-white">
						Now Free
					</span>
				{:else if article.accessMode === 'Free' || article.accessMode === 'free'}
					<span>·</span>
					<span class="inline-flex items-center rounded bg-gray-700 px-1.5 py-0.5 text-[10px] font-semibold text-gray-300">
						Free
					</span>
				{/if}
			</div>
...
{:else}
	...
	<a href="/articles/{article.slug}" class="group block">
		<article class="flex gap-4">
			{#if article.coverImage?.src}
				<div class="relative h-20 w-24 shrink-0 overflow-hidden rounded-lg bg-gray-800 sm:h-24 sm:w-28">
					<div class="absolute inset-0 animate-pulse bg-gray-800"></div>
					<img
						src={article.coverImage.src}
						srcset={article.coverImage.srcset}
			...
			<div class="min-w-0 flex-1">
				...
				{/if}
				<h4 class="mt-0.5 font-display text-sm font-bold leading-snug text-white transition-colors group-hover:text-blue-400 sm:text-base">
					{article.title}
				</h4>
				<div class="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-gray-500">
					...
					{#if article.isPremium}
						<span>·</span>
						<span class="inline-flex items-center gap-0.5 rounded bg-emerald-600 px-1 py-0.5 text-[9px] font-semibold text-white">
					...
					{:else if article.accessMode === 'Premium' || article.accessMode === 'premium'}
						<span>·</span>
						<span class="inline-flex items-center rounded bg-gradient-to-r from-violet-600 to-fuchsia-600 px-1 py-0.5 text-[9px] font-semibold text-white">
							Now Free
						</span>
					{:else if article.accessMode === 'Free' || article.accessMode === 'free'}
						<span>·</span>
						<span class="inline-flex items-center rounded bg-gray-700 px-1 py-0.5 text-[9px] font-semibold text-gray-300">
							Free
						</span>
					{/if}
				</div>
