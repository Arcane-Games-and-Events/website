	...
	>
		...
		<div class="relative z-10 flex items-center gap-2 p-2 sm:hidden">
			...
			<div class="min-w-0 flex-1">
				...
				<div class="mt-0.5 flex items-center gap-2 text-[10px] text-gray-400">
					<span class="flex items-center gap-1">
						...
						<span class="{circuit.colors.text} font-medium">{event.circuit || 'TBA'}</span>
					</span>
					{#if event.format}
						<span>·</span>
			...
			<div class="flex shrink-0 flex-col items-end gap-0.5">
				<span class="text-sm font-bold text-white">${Number(event.price || 0).toFixed(0)}</span>
				<span class="rounded {circuit.colors.bg} px-1.5 py-0.5 text-[9px] font-semibold text-white">
					Sign Up →
				</span>
			</div>
		</div>
		...
		<div class="relative z-10 hidden sm:flex">
			...
			<div class="flex min-w-0 flex-1 items-center gap-4 px-4 py-3">
				...
				<div class="flex shrink-0 flex-col items-end gap-1.5">
					<!-- Price -->
					<span class="text-lg font-bold text-white">${Number(event.price || 0).toFixed(0)}</span>
