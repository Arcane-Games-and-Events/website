...
<!-- Top Navigation Bar -->
<header
	class="fixed top-0 right-0 left-0 z-50 border-b border-white/10 bg-gray-950/80 px-4 backdrop-blur-2xl sm:px-6 lg:px-8"
>
					...
					<div class="relative">
						...
						>
							<div class="relative">
								...
								{#if isPremiumUser}
									<div
										class="absolute -top-0.5 -right-0.5 flex h-3 w-3 items-center justify-center rounded-full bg-emerald-500"
									>
							...
							<svg
								class="h-4 w-4 transition-transform {userMenuOpen ? 'rotate-180' : ''}"
								fill="none"
								stroke="currentColor"
								stroke-width="2"
								viewBox="0 0 24 24"
						...
						{#if userMenuOpen}
							...
							>
								...
								<form method="POST" action="/logout">
									<button
										type="submit"
										class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-red-400 hover:bg-red-500/10"
									>
										<svg
											class="h-4 w-4"
											fill="none"
			...
			<div class="flex-1 overflow-y-auto px-3 py-4">
				<div class="space-y-1">
					{#each mainNavItems as item}
						...
						>
							<svg
								class="h-5 w-5 {checkActive(item.href, $page.url.pathname) ? 'text-blue-400' : ''}"
								fill="none"
								stroke="currentColor"
								stroke-width="1.5"
								viewBox="0 0 24 24"
							...
							{#if checkActive(item.href, $page.url.pathname)}
								<span
									class="ml-auto h-1.5 w-1.5 rounded-full bg-gradient-to-r from-blue-500 to-purple-500"
								></span>
							{/if}
						</a>
					{/each}
				...
				{#if user}
					<div class="mt-6 border-t border-white/10 pt-4">
						<p class="mb-2 px-4 text-xs font-semibold tracking-wider text-gray-500 uppercase">
							Account
						</p>
						<div class="space-y-1">
							<a
			...
			<div class="border-t border-white/10 p-4">
				{#if user}
					...
					<form method="POST" action="/logout">
						<button
							type="submit"
							class="flex w-full items-center justify-center gap-2 rounded-xl bg-red-500/10 px-4 py-2.5 text-sm font-medium text-red-400 transition-colors hover:bg-red-500/20"
						>
							<svg
								class="h-4 w-4"
								fill="none"
