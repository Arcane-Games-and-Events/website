/**
 * Empty Lexical document — used as the initial state for new entries + lessons.
 * Shape matches what `editor.getEditorState().toJSON()` would produce so we can
 * round-trip cleanly.
 */
 ...
 *   { root: ... } object  → as-is
 *   stringified JSON      → parsed (empty doc on parse error)
 */
