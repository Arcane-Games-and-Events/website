 ...
 *   }
 *
 * The `type` string is deliberately `'upload'` (not `'image'`) so both
 * migrated Payload content and CMS-native uploads render through the same
 * branch of `render-lexical.js`.
 */
...
export class ImageUploadNode extends DecoratorNode {
	...
	_renderInto(el) {
		...
		if (!url) {
			...
			el.style.color = 'var(--color-ink, #333)';
			el.style.border = '1px dashed var(--color-line2, #ccc)';
			return;
		}
