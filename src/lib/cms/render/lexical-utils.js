 ...
 *
 * Framework-agnostic — no Svelte, no DOM APIs. Used by both the public
 * renderer (RenderLexical.svelte) and by server-side helpers that need
 * to extract text/headings/reading-time from a stored body.
 */

/** HTML escape — matches the legacy Payload renderer so behavior is identical. */
...
/**
 * Walk a Lexical document tree and return [{ text, id, level }] for each
 * heading. Used for table-of-contents rendering on reader pages.
 */
...
/**
 * ~200 words per minute — matches what Payload computed, so reading-time
 * stays stable for migrated articles.
 */
...
/**
 * Parse a `card:` URL into its parts. Mirrors the logic in the legacy
 * renderer so behavior is identical.
 *
 * Supported separators between name and custom URL: `|`, `%7C`, `::`, `--`
 ...
 * `{ name, pitch }` entries (pitch as 'r' | 'y' | 'b' | null), returned
 * as a Set of JSON-stringified objects. Matches the convention used by
 * the server-side card image resolver.
 */
...
/**
 * Look up a card image entry from the server-resolved map. Map keys are
 * `name.toLowerCase()` and `name.toLowerCase():pitch`.
 */
