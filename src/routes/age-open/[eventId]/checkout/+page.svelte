...
<div class="min-h-screen bg-gray-950">
	<!-- Hero Header -->
	<div
		class="relative overflow-hidden border-b border-gray-800 bg-gradient-to-r from-gray-900 via-gray-900 to-gray-800"
	>
		<div
			class="absolute inset-0 bg-gradient-to-r from-blue-500/5 via-purple-500/5 to-cyan-500/5"
		></div>
		<div class="absolute -top-20 -right-20 h-64 w-64 rounded-full bg-blue-500/10 blur-3xl"></div>
		<div
			class="absolute -bottom-20 -left-20 h-64 w-64 rounded-full bg-purple-500/10 blur-3xl"
		></div>
			...
			<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div>
					<div class="mb-2 flex items-center gap-3">
						...
						<span class="text-sm font-medium tracking-wider text-blue-400 uppercase">Checkout</span>
					</div>
				...
				<div class="flex flex-wrap gap-2">
					{#if data.event.format}
						<span
							class="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-3 py-1.5 text-sm font-medium text-white backdrop-blur-sm"
						>
							<svg
					...
					{#if data.event.circuit}
						<span
							class="inline-flex items-center gap-1.5 rounded-full border border-blue-500/30 bg-blue-500/20 px-3 py-1.5 text-sm font-medium text-blue-400"
						>
					...
					{#if data.hasPremiumDiscount}
						<span
							class="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/20 px-3 py-1.5 text-sm font-medium text-emerald-400"
						>
	...
	<div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
			...
			<div class="space-y-6 lg:col-span-2">
				...
				<div
					class="group relative overflow-hidden rounded-xl border border-gray-800 bg-gray-900/50 p-6 transition-all duration-300 hover:border-gray-700"
				>
					<div
					...
					<div class="relative">
						...
						<div class="grid gap-6 sm:grid-cols-2">
							{#if data.event.eventDate}
								<div
									class="flex items-start gap-4 rounded-lg border border-gray-800 bg-gray-800/30 p-4 transition-colors hover:border-gray-700"
								>
									<div
										class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-purple-500/20"
									>
										<svg
							...
							{#if data.event.location}
								...
								>
									<div
										class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-cyan-500/20"
									>
										<svg
						...
						{#if data.event.description}
							<div class="mt-6 border-t border-gray-800 pt-6">
								<p class="mb-3 text-sm font-medium text-gray-400">About this Event</p>
								<p class="leading-relaxed text-gray-300">{data.event.description}</p>
							</div>
						{/if}
					</div>
				</div>
				...
				{#if data.event.gemIdRequired}
					<div
						class="group relative overflow-hidden rounded-xl border border-amber-500/30 bg-gradient-to-br from-amber-950/30 via-gray-900 to-gray-900 p-6"
					>
						<div
						...
						<div class="relative">
							...
							<div class="rounded-lg border border-gray-700 bg-gray-800/50 p-4">
								...
								<input
									...
									class="w-full rounded-lg border border-gray-700 bg-gray-900 px-4 py-3 text-white transition-all placeholder:text-gray-500 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 focus:outline-none"
								/>
			...
			<div class="lg:col-span-1">
				<div class="sticky top-6 space-y-6">
					<!-- Order Summary Card -->
					<div
						class="relative overflow-hidden rounded-xl border border-gray-800 bg-gray-900/50 p-6"
					>
						<div
							class="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-blue-500/10 blur-2xl"
						></div>
						...
						<div class="relative">
							...
							<div class="space-y-4">
								...
								<div class="rounded-lg border border-gray-800 bg-gray-800/30 p-4">
									<div class="mb-3 flex items-start justify-between gap-3">
										<div class="min-w-0">
											...
											<p class="mt-1 truncate font-semibold text-white">{data.event.title}</p>
										</div>
									...
									<div class="space-y-2 border-t border-gray-700/50 pt-3">
										{#if data.event.eventDate}
											<div class="flex items-center gap-2 text-sm">
												...
												<span class="text-gray-300">{formatShortDate(data.event.eventDate)}</span>
											</div>
										{/if}
										{#if data.event.location}
											<div class="flex items-center gap-2 text-sm">
												...
												<span class="text-gray-300">{data.event.location}</span>
											</div>
										{/if}
										{#if data.event.format}
											<div class="flex items-center gap-2 text-sm">
												...
												<span class="text-gray-300">{data.event.format}</span>
											</div>
										{/if}
									</div>
								</div>
								...
								{#if data.hasPremiumDiscount}
									<div
										class="flex items-center justify-between rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-4 py-3"
									>
							...
							{:else}
								<a
									href="#payment"
									class="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-blue-500 to-purple-600 px-6 py-4 text-lg font-semibold text-white shadow-lg shadow-blue-500/25 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-blue-500/30"
								>
		...
		{#if !data.event.gemIdRequired || canProceed}
			<div id="payment" class="mt-12 scroll-mt-8">
				<div
					class="relative overflow-hidden rounded-xl border border-gray-800 bg-gray-900/50 p-6 sm:p-8"
				>
					<div
						class="absolute -top-20 -left-20 h-64 w-64 rounded-full bg-green-500/5 blur-3xl"
					></div>
					<div
						class="absolute -right-20 -bottom-20 h-64 w-64 rounded-full bg-blue-500/5 blur-3xl"
					></div>

					<div class="relative">
						<div class="mb-8 flex items-center gap-4">
							<div
								class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 shadow-lg shadow-green-500/20"
							>
								<svg
