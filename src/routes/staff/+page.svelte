...
<div class="container mx-auto px-4 py-8 max-w-6xl">
	...
	<!-- Assigned Events -->
	<section>
		<h2 class="mb-4 text-2xl font-bold text-[hsl(var(--foreground))]">My Assigned Events</h2>

		{#if data.assignedEvents.length === 0}
			<div class="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-12 text-center">
				<svg class="mx-auto h-12 w-12 text-[hsl(var(--muted-foreground))] mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{#each data.assignedEvents as event}
					<a
						...
						class="rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 hover:bg-[hsl(var(--muted))] transition-colors"
					>
						<div class="mb-4">
						...
						<div class="space-y-2 mb-4">
							{#if event.location}
								<div class="flex items-center gap-2 text-sm">
									...
									<span class="text-[hsl(var(--foreground))]">{event.location}</span>
								</div>
							{/if}
							...
							{#if event.format}
								<div class="flex items-center gap-2 text-sm">
									...
									<span class="text-[hsl(var(--foreground))]">{event.format}</span>
								</div>
							{/if}
							...
							{#if event.circuit}
								<div class="flex items-center gap-2 text-sm">
									...
									<span class="text-[hsl(var(--foreground))]">{event.circuit}</span>
								</div>
							{/if}
						</div>

						<!-- Stats -->
						<div class="pt-4 border-t border-[hsl(var(--border))] grid grid-cols-2 gap-4">
							<div>
								<p class="text-xs text-[hsl(var(--muted-foreground))]">Tickets Sold</p>
								<p class="text-lg font-bold text-[hsl(var(--foreground))]">{event.ticketCount || 0}</p>
							</div>
							<div>
								<p class="text-xs text-[hsl(var(--muted-foreground))]">Refunds</p>
								<p class="text-lg font-bold text-[hsl(var(--foreground))]">{event.refundCount || 0}</p>
							</div>
						</div>

						<div class="mt-4">
							<span class="inline-flex items-center gap-2 text-sm font-medium text-[hsl(var(--primary))]">
								Manage Event
								<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
