	...
	<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
		<main class="space-y-6 overflow-hidden">
			...
			{:else}
				...
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-3 sm:p-4">
						<div class="flex items-center gap-2 sm:gap-3">
							<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-500/20 sm:h-10 sm:w-10">
								<svg class="h-4 w-4 text-emerald-400 sm:h-5 sm:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
								</svg>
							</div>
							<div class="min-w-0">
								<p class="text-[10px] text-gray-400 sm:text-xs">All-Time Revenue</p>
								<p class="truncate text-lg font-bold text-emerald-400 sm:text-xl">{fmt(data.revenue?.allTime)}</p>
							</div>
						</div>
					</div>
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-3 sm:p-4">
						<div class="flex items-center gap-2 sm:gap-3">
							<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-amber-500/20 sm:h-10 sm:w-10">
								<svg class="h-4 w-4 text-amber-400 sm:h-5 sm:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
								</svg>
							</div>
							<div class="min-w-0">
								<p class="text-[10px] text-gray-400 sm:text-xs">Premium Members</p>
								<p class="truncate text-lg font-bold text-amber-400 sm:text-xl">{num(data.premium?.totalPremium)}</p>
							</div>
						</div>
					</div>
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-3 sm:p-4">
						<div class="flex items-center gap-2 sm:gap-3">
							<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-cyan-500/20 sm:h-10 sm:w-10">
								<svg class="h-4 w-4 text-cyan-400 sm:h-5 sm:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z" />
								</svg>
							</div>
							<div class="min-w-0">
								<p class="text-[10px] text-gray-400 sm:text-xs">Tickets Sold</p>
								<p class="truncate text-lg font-bold text-cyan-400 sm:text-xl">{num(data.tickets?.total)}</p>
							</div>
						</div>
					</div>
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-3 sm:p-4">
						<div class="flex items-center gap-2 sm:gap-3">
							<div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-500/20 sm:h-10 sm:w-10">
								<svg class="h-4 w-4 text-indigo-400 sm:h-5 sm:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
								</svg>
							</div>
							<div class="min-w-0">
								<p class="text-[10px] text-gray-400 sm:text-xs">MRR</p>
								<p class="truncate text-lg font-bold text-indigo-400 sm:text-xl">{fmtD(data.premium?.mrr)}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<!-- REVENUE -->
				<div class="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
					...
					<div class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
						<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
							...
							<p class="mt-1 text-base font-bold text-white sm:text-xl">{fmt(data.revenue?.today)}</p>
							<p class="mt-0.5 text-[10px] text-gray-600 sm:text-xs">{data.revenue?.todayOrders} orders</p>
						</div>
						<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
							<p class="text-[10px] text-gray-500 sm:text-xs">This Week</p>
							<p class="mt-1 text-base font-bold text-white sm:text-xl">{fmt(data.revenue?.week)}</p>
							<p class="mt-0.5 text-[10px] text-gray-600 sm:text-xs">{data.revenue?.weekOrders} orders</p>
						</div>
						<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
							<p class="text-[10px] text-gray-500 sm:text-xs">This Month</p>
							<p class="mt-1 text-base font-bold text-white sm:text-xl">{fmt(data.revenue?.month)}</p>
							<p class="mt-0.5 text-[10px] text-gray-600 sm:text-xs">{data.revenue?.monthOrders} orders</p>
						</div>
						<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
						...
						<div class="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-2.5 sm:p-3">
							...
							<p class="mt-1 text-base font-bold text-emerald-400 sm:text-xl">{fmt(data.revenue?.allTime)}</p>
						</div>
					</div>
							...
							{#each data.revenue?.byType || [] as type}
								...
								<div class="mb-3 last:mb-0">
									<div class="mb-1 flex items-center justify-between">
										<span class="text-xs capitalize text-gray-300 sm:text-sm">{type.type}</span>
										<div class="flex items-center gap-1.5 sm:gap-2">
											<span class="text-xs font-medium text-white sm:text-sm">{fmtD(type.total)}</span>
											<span class="rounded-full bg-white/5 px-1.5 py-0.5 text-[10px] text-gray-500 sm:px-2 sm:text-xs">{type.count}</span>
										</div>
									</div>
						...
						<div>
							...
							{#each data.revenue?.monthlyTrend || [] as month}
								...
								<div class="flex items-center gap-2 py-1.5 sm:gap-3">
									<span class="w-12 shrink-0 text-[10px] text-gray-500 sm:w-16 sm:text-xs">{month.monthName}</span>
									...
									<span class="w-12 shrink-0 text-right text-[10px] font-medium text-gray-300 sm:w-14 sm:text-xs">{fmt(month.total)}</span>
								</div>
							{/each}
						</div>
					</div>
					...
					<div class="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						<div class="mb-4 flex flex-wrap items-center justify-between gap-2 sm:mb-5">
							...
							<div class="flex gap-1.5">
								<span class="rounded-full bg-amber-500/15 px-2 py-0.5 text-[10px] font-medium text-amber-400 sm:px-2.5 sm:text-xs">{data.premium?.paidPremium} paid</span>
								<span class="rounded-full bg-blue-500/15 px-2 py-0.5 text-[10px] font-medium text-blue-400 sm:px-2.5 sm:text-xs">{data.premium?.assignedPremium} staff</span>
							</div>
						</div>
						...
						<div class="mb-4 grid grid-cols-2 gap-2 sm:gap-3">
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<div class="flex items-baseline justify-between">
									...
									<p class="text-[10px] text-gray-600 sm:text-xs">{data.premium?.paidMonthlyActive}m / {data.premium?.paidYearlyActive}y</p>
								</div>
								<p class="mt-1 text-base font-bold text-emerald-400 sm:text-xl">{num(data.premium?.paidActive)}</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<div class="flex items-baseline justify-between">
									<p class="text-[10px] text-gray-500 sm:text-xs">Cancelled</p>
									<p class="text-[10px] text-gray-600 sm:text-xs">{data.premium?.paidMonthlyCancelled}m / {data.premium?.paidYearlyCancelled}y</p>
								</div>
								<p class="mt-1 text-base font-bold text-red-400 sm:text-xl">{num(data.premium?.paidCancelled)}</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<p class="text-[10px] text-gray-500 sm:text-xs">ARR</p>
								<p class="mt-1 text-base font-bold text-emerald-400 sm:text-xl">{fmtD(data.premium?.arr)}</p>
								<p class="mt-0.5 text-[10px] text-gray-600 sm:text-xs">{fmtD(data.premium?.mrr)} / mo</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<p class="text-[10px] text-gray-500 sm:text-xs">Churn Rate</p>
								<p class="mt-1 text-base font-bold sm:text-xl {parseFloat(data.premium?.churn) > 10 ? 'text-red-400' : parseFloat(data.premium?.churn) > 5 ? 'text-amber-400' : 'text-emerald-400'}">{data.premium?.churn}%</p>
								{#if data.premium?.paidPaymentFailed > 0}
									<p class="mt-0.5 text-[10px] text-amber-400 sm:text-xs">{data.premium.paidPaymentFailed} payment failed</p>
								{/if}
							</div>
						</div>
					...
					<div class="min-w-0 overflow-hidden rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						...
						<div class="mb-4 grid grid-cols-2 gap-2 sm:gap-3">
							<div class="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-2.5 sm:p-3">
								...
								<p class="mt-1 text-base font-bold text-emerald-400 sm:text-xl">{fmt(data.tickets?.totalRevenue)}</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<p class="text-[10px] text-gray-500 sm:text-xs">This Month</p>
								<p class="mt-1 text-base font-bold text-white sm:text-xl">{num(data.tickets?.thisMonth)}</p>
								<p class="mt-0.5 text-[10px] text-gray-600 sm:text-xs">{fmtD(data.tickets?.thisMonthRevenue)}</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<p class="text-[10px] text-gray-500 sm:text-xs">Avg Price</p>
								<p class="mt-1 text-base font-bold text-white sm:text-xl">{fmtD(data.tickets?.avgTicketPrice)}</p>
							</div>
							<div class="rounded-xl border border-white/5 bg-gray-800/40 p-2.5 sm:p-3">
								<div class="flex items-baseline justify-between">
								...
								<p class="mt-1 text-base font-bold text-red-400 sm:text-xl">{num(data.tickets?.refunded)}</p>
							</div>
						</div>
							...
							<div>
								...
								{#each data.tickets?.byCircuit || [] as c}
									...
									<div class="flex items-center gap-1.5 py-1.5 sm:gap-2.5">
										...
										<span class="min-w-0 flex-1 truncate text-xs text-gray-300 sm:text-sm">{c.circuit}</span>
										<span class="shrink-0 rounded-full bg-white/5 px-1.5 py-0.5 text-[10px] text-gray-400 sm:px-2 sm:text-xs">{c.count}</span>
										<span class="w-14 shrink-0 text-right text-[10px] font-medium text-emerald-400 sm:w-16 sm:text-xs">{fmtD(c.revenue)}</span>
									</div>
								{/each}
							</div>
							<div>
								...
								{#each (data.tickets?.topEvents || []).slice(0, 5) as e, i}
									<div class="flex items-center gap-1.5 py-1.5 sm:gap-2.5">
										...
										<span class="min-w-0 flex-1 truncate text-xs text-gray-300 sm:text-sm">{e.title || 'Unknown'}</span>
										<span class="shrink-0 rounded-full bg-white/5 px-1.5 py-0.5 text-[10px] text-gray-400 sm:px-2 sm:text-xs">{e.count}</span>
										<span class="w-14 shrink-0 text-right text-[10px] font-medium text-emerald-400 sm:w-16 sm:text-xs">{fmtD(e.revenue)}</span>
									</div>
								{/each}
							</div>
						</div>
					...
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						...
						{#each data.tickets?.monthlyTrend || [] as month}
							...
							<div class="flex items-center gap-2 py-1.5 sm:gap-3">
								...
								<span class="w-6 shrink-0 text-right text-[10px] font-medium text-gray-300 sm:w-8 sm:text-xs">{month.count}</span>
								<span class="w-12 shrink-0 text-right text-[10px] text-emerald-400 sm:w-14 sm:text-xs">{fmt(month.revenue)}</span>
							</div>
						{/each}
					</div>
					...
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						...
						{#each data.premium?.monthlyTrend || [] as month}
							...
							<div class="flex items-center gap-2 py-1.5 sm:gap-3">
								...
								<span class="w-6 shrink-0 text-right text-[10px] font-medium text-gray-400 sm:text-xs">{month.total}</span>
							</div>
						{/each}
					</div>
				</div>
				...
				<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						<div class="mb-4 flex items-center justify-between">
							...
							<span class="rounded-full bg-cyan-500/15 px-2 py-0.5 text-[10px] font-medium text-cyan-400 sm:px-2.5 sm:text-xs">{data.events?.upcoming} upcoming</span>
						</div>
					...
					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:p-5">
						...
						{#each (data.players?.topPlayers || []).slice(0, 7) as p, i}
							<div class="flex items-center gap-1.5 py-1.5 sm:gap-2.5">
								...
								<span class="min-w-0 flex-1 truncate text-xs text-gray-300 sm:text-sm">{p.name}</span>
								<span class="shrink-0 text-[10px] text-gray-500 sm:text-xs">{p.matchesWon}W-{p.matchesPlayed - p.matchesWon}L</span>
								<span class="shrink-0 text-xs font-semibold text-white sm:text-sm">{p.points}</span>
							</div>
						{/each}
					</div>

					<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4 sm:col-span-2 sm:p-5 lg:col-span-1">
						<div class="mb-4 flex items-center gap-3">
						...
						{#each (data.customers?.topCustomers || []).slice(0, 7) as c, i}
							<div class="flex items-center gap-1.5 py-1.5 sm:gap-2.5">
								...
								<span class="min-w-0 flex-1 truncate text-xs text-gray-300 sm:text-sm">{c.email}</span>
								<span class="shrink-0 rounded-full bg-white/5 px-1.5 py-0.5 text-[10px] text-gray-500 sm:px-2 sm:text-xs">{c.orderCount}</span>
								<span class="shrink-0 text-xs font-semibold text-emerald-400 sm:text-sm">{fmtD(c.totalSpent)}</span>
							</div>
						{/each}
					</div>
				</div>
