<script>
	...
	const premiumMetrics = $derived(() => {
		...
		const activeMonthly =
			breakdown.find((s) => s.type === 'monthly' && s.status === 'active')?.count || 0;
		const activeYearly =
			breakdown.find((s) => s.type === 'yearly' && s.status === 'active')?.count || 0;
		const cancelledMonthly =
			breakdown.find((s) => s.type === 'monthly' && s.status === 'cancelled')?.count || 0;
		const cancelledYearly =
			breakdown.find((s) => s.type === 'yearly' && s.status === 'cancelled')?.count || 0;
		const failedPayments = breakdown
	...
	<div class="mb-6 flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-gray-900/50 p-1">
		{#each sections as section}
			<button
				...
				class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium whitespace-nowrap transition-all {activeSection ===
				section.id
					...
					<div class="space-y-4">
						{#each [1, 2, 3, 4, 5] as _}
							<div class="flex items-center gap-3">
								...
								<div class="h-4 flex-1 rounded bg-gray-700/30"></div>
	...
	{:else}
		...
		{#if activeSection === 'overview'}
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					<div class="flex items-center justify-between">
						...
						{#if data.revenue?.growth}
							<span
								class="rounded-full bg-white/5 px-2 py-1 text-xs font-medium {getGrowthColor(
									data.revenue.growth
								)}"
							>
								{getGrowthIcon(data.revenue.growth)}
								{Math.abs(parseFloat(data.revenue.growth))}%
							</span>
						{/if}
					</div>
					...
					<div class="flex items-center justify-between">
						...
						{#if data.users?.growth}
							...
							>
								...
								{Math.abs(parseFloat(data.users.growth))}%
							</span>
						{/if}
					</div>
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					<div class="flex items-center justify-between">
						...
						{#if data.tickets?.growth}
							...
							>
								{getGrowthIcon(data.tickets.growth)}
								{Math.abs(parseFloat(data.tickets.growth))}%
							</span>
						{/if}
					</div>
			...
			<div class="mb-8 grid gap-4 sm:grid-cols-2">
				<!-- Premium Members -->
				<div
					class="rounded-xl border border-amber-500/30 bg-gradient-to-br from-amber-500/10 to-yellow-500/5 p-5"
				>
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="text-2xl font-bold text-white">
									{formatNumber(data.users?.activeSubscribers)}
								</p>
					...
					<div class="mt-4 flex items-center gap-4 border-t border-white/10 pt-4">
						<div class="flex-1">
							...
							<p class="text-lg font-semibold text-amber-400">
								{formatCurrency(premiumMetrics().monthlyMRR)}
							</p>
						</div>
						<div class="flex-1">
							<p class="text-xs text-gray-500">Annual ARR</p>
							<p class="text-lg font-semibold text-white">
								{formatCurrency(premiumMetrics().annualARR)}
							</p>
						</div>
					</div>
				</div>
			...
			<div class="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4">
					<p class="text-xs font-medium tracking-wider text-gray-500 uppercase">Today</p>
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4">
					<p class="text-xs font-medium tracking-wider text-gray-500 uppercase">This Week</p>
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4">
					<p class="text-xs font-medium tracking-wider text-gray-500 uppercase">This Month</p>
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-4">
					<p class="text-xs font-medium tracking-wider text-gray-500 uppercase">Year to Date</p>
						...
						{#each data.revenue?.byType || [] as type}
							...
							<div>
								<div class="mb-1 flex items-center justify-between">
									<span class="text-sm text-gray-300 capitalize">{type.type}</span>
								...
								<div class="h-2 overflow-hidden rounded-full bg-gray-800">
									<div
										class="h-full rounded-full {type.type === 'ticket'
											? 'bg-blue-500'
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-6">
					...
					<div class="space-y-3">
						{#each data.users?.byRole || [] as role}
							...
							<div>
								<div class="mb-1 flex items-center justify-between">
									<span class="text-sm text-gray-300 capitalize">{role.role}</span>
									<span class="text-sm font-medium text-white">{formatNumber(role.count)}</span>
								</div>
								<div class="h-2 overflow-hidden rounded-full bg-gray-800">
									<div
										class="h-full rounded-full {role.role === 'admin'
											? 'bg-red-500'
											: role.role === 'premium'
												? 'bg-amber-500'
												: role.role === 'writer'
													? 'bg-cyan-500'
													: role.role === 'tournament staff'
														? 'bg-purple-500'
														: 'bg-gray-500'}"
		...
		{#if activeSection === 'revenue'}
			...
			<div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					<p class="mt-2 text-2xl font-bold text-white">
						{formatCurrency(data.customers?.avgOrderValue)}
					</p>
				</div>
			</div>
					...
					<div class="flex h-full items-end gap-1">
						{#each data.revenue?.dailyTrend || [] as day}
							...
							<div class="group relative flex-1">
								...
								></div>
								<div
									class="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 hidden -translate-x-1/2 rounded bg-gray-800 px-2 py-1 text-xs text-white group-hover:block"
								>
					...
					<div class="space-y-4">
						{#each data.revenue?.byType || [] as type}
							...
							>
								<div class="flex items-center gap-3">
									<div
										class="flex h-10 w-10 items-center justify-center rounded-lg {type.type ===
										'ticket'
											? 'bg-blue-500/20'
											: type.type === 'subscription'
												? 'bg-green-500/20'
									...
									<div>
										<p class="font-medium text-white capitalize">{type.type}</p>
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					{#if data.users?.growth}
						<p class="mt-1 text-xs {getGrowthColor(data.users.growth)}">
							{getGrowthIcon(data.users.growth)}
							{data.users.growth}% vs last month
						</p>
					{/if}
				</div>
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					<div class="space-y-4">
						{#each data.users?.byRole || [] as role}
							...
							>
								<div class="flex items-center gap-3">
									<div
										class="flex h-10 w-10 items-center justify-center rounded-full {role.role ===
										'admin'
									...
									<p class="font-medium text-white capitalize">{role.role}</p>
								</div>
			...
			<div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				>
					...
					<p class="mt-2 text-3xl font-bold text-white">
						{formatNumber(premiumMetrics().totalActive)}
					</p>
					<p class="mt-1 text-xs text-amber-400">
						{formatNumber(premiumMetrics().activeMonthly)} monthly, {formatNumber(
							premiumMetrics().activeYearly
						)} yearly
					</p>
				</div>
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					<p
						class="mt-2 text-2xl font-bold {parseFloat(premiumMetrics().churnRate) > 10
							? 'text-red-400'
							: parseFloat(premiumMetrics().churnRate) > 5
								? 'text-yellow-400'
								: 'text-green-400'}"
					>
					...
					<p class="mt-1 text-xs text-gray-500">
						{formatNumber(premiumMetrics().totalCancelled)} cancelled
					</p>
				</div>
			</div>
						...
						<div class="rounded-lg border border-white/5 bg-white/5 p-4">
							<div class="mb-3 flex items-center justify-between">
								...
								<p class="text-lg font-bold text-blue-400">
									{formatNumber(premiumMetrics().activeMonthly)}
								</p>
							</div>
							<div class="flex gap-4 text-xs">
								<span class="text-green-400"
									>{formatNumber(premiumMetrics().activeMonthly)} active</span
								>
								<span class="text-red-400"
									>{formatNumber(premiumMetrics().cancelledMonthly)} cancelled</span
								>
							</div>
						</div>
						...
						<div class="rounded-lg border border-white/5 bg-white/5 p-4">
							<div class="mb-3 flex items-center justify-between">
								...
								<p class="text-lg font-bold text-amber-400">
									{formatNumber(premiumMetrics().activeYearly)}
								</p>
							</div>
							<div class="flex gap-4 text-xs">
								<span class="text-green-400"
									>{formatNumber(premiumMetrics().activeYearly)} active</span
								>
								<span class="text-red-400"
									>{formatNumber(premiumMetrics().cancelledYearly)} cancelled</span
								>
							</div>
						</div>
						...
						{#if premiumMetrics().failedPayments > 0}
							<div class="rounded-lg border border-red-500/30 bg-red-500/10 p-4">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-3">
										<div
											class="flex h-10 w-10 items-center justify-center rounded-lg bg-red-500/20"
										>
											<svg
									...
									<p class="text-lg font-bold text-red-400">
										{formatNumber(premiumMetrics().failedPayments)}
									</p>
								</div>
							</div>
						{/if}
							...
							<div class="space-y-3">
								<div>
									<div class="mb-1 flex items-center justify-between">
										...
										<span class="text-sm font-medium text-white"
											>{formatCurrency(premiumMetrics().activeMonthly * 9.99)}</span
										>
									</div>
									<div class="h-2 overflow-hidden rounded-full bg-gray-800">
										<div
											class="h-full rounded-full bg-blue-500"
											style="width: {premiumMetrics().monthlyMRR > 0
												? ((premiumMetrics().activeMonthly * 9.99) / premiumMetrics().monthlyMRR) *
													100
												: 0}%"
										></div>
									</div>
								...
								<div>
									<div class="mb-1 flex items-center justify-between">
										...
										<span class="text-sm font-medium text-white"
											>{formatCurrency(premiumMetrics().activeYearly * 8.25)}</span
										>
									</div>
									<div class="h-2 overflow-hidden rounded-full bg-gray-800">
										<div
											...
											style="width: {premiumMetrics().monthlyMRR > 0
												? ((premiumMetrics().activeYearly * 8.25) / premiumMetrics().monthlyMRR) *
													100
												: 0}%"
										></div>
									</div>
						...
						<div class="border-t border-white/10 pt-4">
							...
							<div class="grid grid-cols-2 gap-4">
								<div class="rounded-lg bg-white/5 p-3">
									...
									<p class="text-lg font-semibold text-white">
										{formatCurrency(
											premiumMetrics().totalActive > 0
												? premiumMetrics().monthlyMRR / premiumMetrics().totalActive
												: 0
										)}
									</p>
								...
								<div class="rounded-lg bg-white/5 p-3">
									...
									<p class="text-lg font-semibold text-white">
										{premiumMetrics().totalActive > 0
											? Math.round(
													(premiumMetrics().activeYearly / premiumMetrics().totalActive) * 100
												)
											: 0}%
									</p>
								...
								<div class="flex-1">
									<div class="mb-1 flex items-center justify-between">
										...
										<span class="text-xs text-green-400"
											>{formatNumber(premiumMetrics().totalActive)}</span
										>
									</div>
									<div class="h-3 overflow-hidden rounded-full bg-gray-800">
										<div
											...
											style="width: {premiumMetrics().totalActive +
												premiumMetrics().totalCancelled >
											0
												? (premiumMetrics().totalActive /
														(premiumMetrics().totalActive + premiumMetrics().totalCancelled)) *
													100
												: 100}%"
										></div>
									</div>
								...
								<div class="flex-1">
									<div class="mb-1 flex items-center justify-between">
										...
										<span class="text-xs text-red-400"
											>{formatNumber(premiumMetrics().totalCancelled)}</span
										>
									</div>
									<div class="h-3 overflow-hidden rounded-full bg-gray-800">
										<div
						...
						<table class="w-full">
							<thead>
								<tr class="border-b border-gray-800">
									...
									>
									<th
										class="pb-3 text-right text-xs font-medium tracking-wider text-gray-500 uppercase"
										>Count</th
									>
								</tr>
							</thead>
							<tbody class="divide-y divide-gray-800">
								{#each premiumMetrics().breakdown as sub}
									<tr>
										<td class="py-3 text-sm text-white capitalize">{sub.type || 'Unknown'}</td>
										<td class="py-3">
											<span
										...
										<td class="py-3 text-right text-sm font-medium text-white"
											>{formatNumber(sub.count)}</td
										>
									</tr>
								{/each}
							</tbody>
		...
		{#if activeSection === 'events'}
			...
			<div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					<p class="mt-2 text-2xl font-bold text-white">
						{formatCurrency(data.tickets?.totalRevenue)}
					</p>
				</div>
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					{#if data.tickets?.growth}
						<p class="mt-1 text-xs {getGrowthColor(data.tickets.growth)}">
							{getGrowthIcon(data.tickets.growth)}
							{data.tickets.growth}% vs last month
						</p>
					{/if}
				</div>
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
						...
						{#each data.tickets?.byCircuit || [] as circuit}
							<div class="rounded-lg border border-white/5 bg-white/5 p-4">
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-3">
										<div
											class="h-3 w-3 rounded-full {getCircuit(circuit.circuit).colors.dot}"
										></div>
								...
								<p class="mt-1 text-sm text-emerald-400">
									{formatCurrency(circuit.revenue)} revenue
								</p>
							</div>
						{/each}
					</div>
					...
					<div class="space-y-3">
						{#each data.tickets?.topEvents || [] as event, i}
							...
							>
								<div class="flex items-center gap-3">
									<span
										class="flex h-8 w-8 items-center justify-center rounded-full bg-purple-500/20 text-sm font-bold text-purple-400"
									>
										{i + 1}
									</span>
									<div>
										...
										<p class="text-xs text-gray-400">
											{event.circuit} &middot; {event.count} tickets
										</p>
									</div>
								</div>
		...
		{#if activeSection === 'players'}
			...
			<div class="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				...
				<div class="rounded-xl border border-white/10 bg-gray-900/50 p-5">
					...
					<p class="mt-2 text-2xl font-bold text-white">
						{formatNumber((data.tickets?.total || 0) - (data.tickets?.refunded || 0))}
					</p>
				</div>
			</div>
					...
					<div class="space-y-3">
						{#each data.players?.topPlayers || [] as player, i}
							...
							>
								<div class="flex items-center gap-3">
									<span
										class="flex h-8 w-8 items-center justify-center rounded-full {i === 0
											? 'bg-yellow-500/20 text-yellow-400'
											: i === 1
												? 'bg-gray-400/20 text-gray-300'
												: i === 2
													? 'bg-amber-600/20 text-amber-500'
													: 'bg-rose-500/20 text-rose-400'} text-sm font-bold"
									>
										{i + 1}
									</span>
									<div>
										...
										<p class="text-xs text-gray-400">
											{player.circuit} &middot; {player.matchesWon}W-{player.matchesPlayed -
												player.matchesWon}L
										</p>
									</div>
								</div>
