...
<svelte:head>
	<title>{data.customer ? `${data.customer.firstName} ${data.customer.lastName}` : data.customerEmail} | Admin</title>
</svelte:head>
...
<div class="px-4 py-8 sm:px-6 lg:px-8">
		<div class="mx-auto max-w-6xl">
			...
			<nav class="mb-6 flex items-center gap-2 text-sm">
				...
				<span class="text-white">{data.customer ? `${data.customer.firstName || ''} ${data.customer.lastName || ''}`.trim() || data.customerEmail : data.customerEmail}</span>
			</nav>
			...
			<div class="p-6">
				<div class="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
					...
					<div class="flex items-start gap-4">
						<div
							class="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br text-2xl font-bold text-white {getAvatarGradient(data.customer?.role)}"
						>
							{#if data.customer}
						...
						<div>
							...
							<div class="mt-2 flex flex-wrap items-center gap-2">
								{#if data.customer}
									<span class="inline-flex items-center rounded-full border px-3 py-1 text-sm font-medium capitalize {getRoleBadgeClasses(data.customer.role)}">
										{data.customer.role}
									</span>
									{#if data.customer.gemId}
										<span class="inline-flex items-center rounded-full bg-amber-500/20 px-3 py-1 text-sm text-amber-400">
											GEM: {data.customer.gemId}
										</span>
									{/if}
								{:else}
									<span class="inline-flex items-center rounded-full border border-gray-600 bg-gray-700 px-3 py-1 text-sm font-medium text-gray-400">
										No Account
									</span>
								{/if}
							</div>
					...
					<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
						<div class="grid grid-cols-2 gap-6 sm:grid-cols-4">
							<div>
								...
								<span class="mt-1 inline-flex items-center rounded-full px-2.5 py-1 text-sm font-medium capitalize {getSubscriptionStatusClasses(data.customer.subscriptionStatus)}">
									{data.customer.subscriptionStatus || 'None'}
								</span>
							</div>
							<div>
								<p class="text-sm text-gray-400">Type</p>
								<p class="mt-1 font-medium capitalize text-white">{data.customer.subscriptionType || 'N/A'}</p>
							</div>
							<div>
								<p class="text-sm text-gray-400">Start Date</p>
								<p class="mt-1 font-medium text-white">{formatDate(data.customer.subscriptionStartDate)}</p>
							</div>
							<div>
								<p class="text-sm text-gray-400">Next Billing</p>
								<p class="mt-1 font-medium text-white">{formatDate(data.customer.nextBillingDate)}</p>
							</div>
						</div>
						{#if data.customer.subscriptionStatus === 'active'}
							<form
								...
								use:enhance={() => {
									if (!confirm('Are you sure you want to cancel this subscription? The user will be downgraded to the free tier.')) {
										return () => {};
									}
									cancelLoading = true;
									return async ({ update }) => {
					...
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<p class="text-lg font-semibold text-white">
								...
								<span class="text-sm font-normal text-gray-400">({formatCurrency(data.ordersByType.ticket.total)})</span>
							</p>
						</div>
					</div>
				</div>
					...
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<p class="text-lg font-semibold text-white">
								...
								<span class="text-sm font-normal text-gray-400">({formatCurrency(data.ordersByType.course.total)})</span>
							</p>
						</div>
					</div>
				</div>
					...
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<p class="text-lg font-semibold text-white">
								...
								<span class="text-sm font-normal text-gray-400">({formatCurrency(data.ordersByType.subscription.total)})</span>
							</p>
						</div>
					</div>
				</div>
						...
						<table class="w-full">
							<thead class="bg-gray-800/50">
								<tr>
									...
									<th class="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider text-gray-400">Amount</th>
							...
							<tbody class="divide-y divide-gray-800">
								{#each data.orders as order}
									<tr class="transition-colors hover:bg-gray-800/50">
										...
										<td class="px-6 py-4 text-right text-sm font-medium text-white">{formatCurrency(order.amount)}</td>
						...
						<table class="w-full">
							<thead class="bg-gray-800/50">
								<tr>
									...
									<th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-gray-400">Ticket Code</th>
							...
							<tbody class="divide-y divide-gray-800">
								{#each data.tickets as { ticket, event }}
									<tr class="transition-colors hover:bg-gray-800/50">
										...
										<td class="px-6 py-4">
											{#if ticket.refunded}
												<span class="inline-flex items-center rounded-full bg-red-500/20 px-2.5 py-1 text-xs font-medium text-red-400">
													Refunded
												</span>
											{:else if event?.eventDate && new Date(event.eventDate) < new Date()}
												<span class="inline-flex items-center rounded-full bg-gray-500/20 px-2.5 py-1 text-xs font-medium text-gray-400">
													Past Event
												</span>
											{:else}
												<span class="inline-flex items-center rounded-full bg-green-500/20 px-2.5 py-1 text-xs font-medium text-green-400">
													Active
												</span>
											{/if}
										</td>
