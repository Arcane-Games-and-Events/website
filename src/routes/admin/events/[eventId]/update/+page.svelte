...
<div class="px-4 py-8 sm:px-6 lg:px-8">
	<div class="mx-auto max-w-7xl">
		...
		{#if form?.success}
			<div class="mb-6 rounded-xl border border-green-500/30 bg-green-500/10 p-4">
				...
				{#if form.details}
					<ul class="ml-8 mt-2 list-inside list-disc text-sm text-green-300">
		...
		<div class="mb-6 flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-gray-900/50 p-1">
			<button
				...
				class="whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all {activeTab === 'import'
			...
			<button
				...
				class="whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all {activeTab === 'results'
					? 'bg-indigo-500/20 text-indigo-400'
					: 'text-gray-400 hover:bg-white/5 hover:text-white'}"
			>
			...
			<button
				...
				class="whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all {activeTab === 'decklists'
					? 'bg-indigo-500/20 text-indigo-400'
					: 'text-gray-400 hover:bg-white/5 hover:text-white'}"
			>
			...
			<button
				...
				class="whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all {activeTab === 'finalize'
					? 'bg-indigo-500/20 text-indigo-400'
					: 'text-gray-400 hover:bg-white/5 hover:text-white'}"
			>
				Finalize
	...
	{#if activeTab === 'import'}
		<div class="max-w-3xl mx-auto space-y-6">
			<div class="rounded-xl bg-gray-950 border shadow-md p-8">
					...
					<form method="POST" action="?/processCSV" enctype="multipart/form-data" class="space-y-6">
						<div>
							...
							<input
								...
								class="w-full rounded-xl border border-white/10 bg-gray-900 px-4 py-3 text-gray-100 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-white file:text-gray-900 hover:file:opacity-90"
							/>
						</div>
						...
						<button
							...
							class="w-full rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 text-lg font-semibold text-gray-900 hover:opacity-90 transition-opacity disabled:opacity-50"
						>
			...
			<div class="rounded-xl bg-gray-950 border shadow-md p-6">
	...
	{#if activeTab === 'results'}
		<div class="space-y-6">
			...
			{#if !isCompleted}
				<div class="flex justify-end">
					<button
						...
						class="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-2 text-sm font-medium text-gray-900 hover:opacity-90 transition-opacity"
					>
						+ Add Result
					</button>
				</div>
			...
			<!-- Result Form Modal -->
			{#if showResultForm}
				<div class="rounded-xl bg-gray-900 border border-white/10 p-6">
			...
			<!-- Results List -->
			<div class="rounded-xl bg-gray-950 border shadow-md">
...
3x Fyendal's Spring Tunic"
								class="w-full rounded-xl border border-white/10 bg-gray-950 px-4 py-2.5 text-gray-100 font-mono text-sm focus:outline-none focus:ring-2 focus:ring-gray-300"
							></textarea>
						</div>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{#if data.existingDecklists?.length === 0}
					<div class="col-span-full rounded-xl bg-gray-950 border shadow-md p-12 text-center">
						<svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				{:else}
					{#each data.existingDecklists as decklist}
						<div class="rounded-xl bg-gray-950 border shadow-md p-4">
						...
						<div class="rounded-lg bg-green-500/10 border border-green-500/30 p-4">
							...
							<form method="POST" action="?/finalizeEvent">
								<button
									...
									class="w-full rounded-xl bg-green-600 px-6 py-3 text-lg font-semibold text-white hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
									on:click|preventDefault={(e) => {
