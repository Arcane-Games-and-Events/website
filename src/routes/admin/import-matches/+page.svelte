	...
	<div class="mx-auto max-w-7xl">
		...
		<div class="relative mb-8 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-teal-900/30 via-gray-900 to-gray-950 p-6 shadow-2xl shadow-teal-500/5">
			...
			<div class="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
				...
				<!-- Stats -->
				<div class="flex flex-wrap items-center gap-4 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
						...
						<div class="flex items-center justify-between">
							...
							<div class="flex flex-col items-end gap-1">
								<div class="flex items-center gap-2 text-sm">
									...
									<span class="rounded-full bg-green-500/20 px-2 py-0.5 text-xs font-medium text-green-400">{circuit.eventsWithMatches}</span>
									<span class="rounded-full bg-red-500/20 px-2 py-0.5 text-xs font-medium text-red-400">{circuit.eventsMissing}</span>
								</div>
								<div class="flex items-center gap-2 text-sm">
									<span class="text-xs text-gray-500">Heroes:</span>
									<span class="rounded-full bg-purple-500/20 px-2 py-0.5 text-xs font-medium text-purple-400">{circuit.eventsWithHeroes}</span>
									<span class="rounded-full bg-orange-500/20 px-2 py-0.5 text-xs font-medium text-orange-400">{circuit.eventsMissingHeroes}</span>
								</div>
							</div>
						</div>
			...
			<div class="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-gray-950/95 shadow-2xl backdrop-blur-sm lg:left-64">
				<div class="mx-auto max-w-5xl p-4">
					...
					{:else}
						...
						<div class="mb-3 rounded-lg border border-white/10 bg-white/5 p-3">
							<p class="mb-1 text-xs font-medium text-gray-400">Expected CSV columns for hero data:</p>
							<p class="font-mono text-xs text-gray-500">Name, Hero, GEM ID (optional), Country/Region (optional)</p>
							<p class="mt-1 text-xs text-gray-500">Each row should represent one player and their hero for this event</p>
						</div>
						...
						>
							...
							{#if selectedEvent.heroCount > 0}
								<div class="rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-3 py-2">
									<p class="text-xs text-yellow-400">
										<strong>Warning:</strong> This will replace {selectedEvent.heroCount} existing hero entries
									</p>
								</div>
							{/if}
							...
							<button
								type="submit"
								class="whitespace-nowrap rounded-lg bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-2 font-semibold text-white shadow-lg shadow-purple-500/25 transition-all hover:from-purple-500 hover:to-pink-500"
							>
								Import Heroes
							</button>
						</form>
