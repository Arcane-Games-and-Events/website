...
<div class="mx-auto max-w-5xl px-4 py-8">
		...
		<div class="space-y-4">
			{#each data.assignedEvents as event}
				...
				<a
					...
					class="group block overflow-hidden rounded-xl border border-white/10 bg-gray-900/50 transition-all hover:bg-gray-900 hover:border-white/20"
				>
					<div class="flex">
							...
							<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
								...
								<div class="flex-1">
									<div class="flex items-center gap-3 mb-2">
										...
										<span class="rounded-full {status.bg} px-2 py-0.5 text-xs font-medium {status.text}">
											{status.label}
										</span>
									</div>
	...
	{:else}
		<div class="rounded-xl border border-white/10 bg-gray-900/50 p-12 text-center">
			...
			<h3 class="text-lg font-medium text-white mb-2">No Events Assigned</h3>
			<p class="text-gray-400">You haven't been assigned to any events yet.</p>
			<p class="text-sm text-gray-500 mt-1">Contact an administrator to be assigned to an event.</p>
		</div>
	{/if}
</div>
