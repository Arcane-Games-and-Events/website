	...
	<div class="mx-auto max-w-xl px-4">
		...
		<div class="mb-6 rounded-lg border border-amber-500/50 bg-amber-500/10 p-4">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-sm text-amber-200/80">This page creates a real 7-day subscription for webhook testing</p>
				</div>
			</div>
		</div>
		...
		{#if !user}
			...
			<div class="rounded-xl border border-gray-800 bg-gray-900/50 p-8 text-center">
				...
				<a href="/login" class="mt-6 inline-block rounded-lg bg-blue-600 px-6 py-3 font-medium text-white hover:bg-blue-500 transition-colors">
					Log In
				</a>
			</div>
		{:else if user.role === 'premium' && user.subscriptionStatus === 'active'}
			...
			<div class="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-center">
				...
				<p class="mt-1 text-sm text-gray-500">
					Next billing: {user.nextBillingDate ? new Date(user.nextBillingDate).toLocaleDateString() : 'N/A'}
				</p>
				<a href="/account" class="mt-6 inline-block rounded-lg bg-gray-700 px-6 py-3 font-medium text-white hover:bg-gray-600 transition-colors">
					Go to Account
				</a>
			</div>
		{:else}
			...
			<div class="mb-6 rounded-xl border border-blue-500/30 bg-blue-500/10 p-6">
				<h3 class="text-lg font-semibold text-white mb-3">Test Subscription Details</h3>
				<div class="space-y-2 text-sm">
					<div class="flex justify-between">
						<span class="text-gray-400">Amount:</span>
						<span class="text-white font-medium">$0.01</span>
					</div>
					<div class="flex justify-between">
						<span class="text-gray-400">Billing Interval:</span>
						<span class="text-white font-medium">Every 7 days</span>
					</div>
					<div class="flex justify-between">
						<span class="text-gray-400">Total Occurrences:</span>
						<span class="text-white font-medium">5 payments</span>
					</div>
					<div class="flex justify-between">
						<span class="text-gray-400">First Webhook:</span>
						<span class="text-emerald-400 font-medium">In 7 days</span>
					</div>
				</div>
