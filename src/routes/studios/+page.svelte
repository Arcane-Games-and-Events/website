...
<div class="min-h-screen overflow-x-hidden">
	...
	<section class="relative px-3 pt-4 sm:px-6 sm:pt-6 lg:px-8">
		<div class="mx-auto max-w-7xl">
			<div class="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gray-900">
				<!-- Background image -->
				<img
				...
				<div class="absolute inset-0 bg-gradient-to-r from-gray-900 via-gray-900/80 to-gray-900/70 sm:via-gray-900/70 sm:to-gray-900/60 md:to-transparent"></div>

				<!-- Content -->
				<div class="relative z-10 flex items-center px-4 py-8 sm:px-6 sm:py-10 md:px-10 md:py-14 lg:px-12 lg:py-16">
					<div class="max-w-2xl">
						<!-- Badge -->
						<div class="mb-2 flex items-center gap-2 sm:mb-3 md:mb-4">
							<div class="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/10 px-2.5 py-1 sm:px-3 sm:py-1.5 backdrop-blur-sm">
								<svg class="h-3 w-3 sm:h-3.5 sm:w-3.5 text-red-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" d={icons.playCircle} />
								</svg>
								<span class="text-[10px] sm:text-xs font-semibold tracking-wider text-red-400 uppercase">Watch & Listen</span>
							</div>
						</div>

						<!-- Title -->
						<h1 class="mb-1.5 text-2xl font-bold text-white sm:mb-2 sm:text-3xl md:text-4xl lg:text-5xl">
							AGE Studios
						</h1>
						<p class="mb-1.5 bg-gradient-to-r from-red-400 to-purple-400 bg-clip-text text-base font-semibold text-transparent sm:mb-2 sm:text-lg md:mb-4 md:text-2xl">
							Premium Content Hub
						</p>
						...
						<div class="flex flex-wrap gap-2 sm:gap-3">
							...
							{#if !isPremiumUser}
								...
								>
									<svg class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-400" fill="currentColor" viewBox="0 0 24 24">
	...
	<section id="content" class="scroll-mt-6 overflow-hidden px-3 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
		<div class="mx-auto max-w-7xl">
			<div class="grid w-full gap-6 sm:gap-8 lg:grid-cols-3 lg:gap-10">
				...
				<div class="min-w-0 space-y-8 sm:space-y-10 lg:col-span-2">
					...
					<!-- Tournament VODs -->
					<div>
						<div class="mb-4 flex items-center justify-between border-b border-white/10 pb-2.5 sm:mb-5 sm:pb-3">
								...
								{#each vods as vodItem (vodItem.id)}
									...
									>
										...
										<div class="min-w-0 overflow-hidden p-2 sm:p-3">
											<h3 class="mb-0.5 line-clamp-2 text-xs font-semibold leading-tight text-white transition-colors group-hover:text-red-400 sm:mb-1 sm:text-sm">
												{vodItem.title}
											</h3>
											<div class="flex flex-wrap items-center gap-1 text-[10px] text-gray-500 sm:gap-2 sm:text-xs">
												{#if vodItem.player1Name && vodItem.player2Name}
													<span class="hidden rounded bg-white/5 px-1.5 py-0.5 sm:inline">{vodItem.player1Name} vs {vodItem.player2Name}</span>
												{/if}
						...
						{:else}
							<!-- No VODs at all -->
							<div class="flex flex-col items-center justify-center rounded-lg border border-white/5 bg-gray-900/40 px-4 py-10 text-center sm:rounded-xl sm:py-12">
								<svg class="mb-2 h-8 w-8 text-gray-600 sm:mb-3 sm:h-10 sm:w-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" d={icons.playCircle} />
								</svg>
								<p class="text-xs font-medium text-gray-400 sm:text-sm">Tournament VODs coming soon</p>
								<p class="mt-1 text-[10px] text-gray-500 sm:text-xs">Premium members will get full access to all tournament recordings</p>
							</div>
						{/if}
					</div>
					...
					{#if currentPodcast}
						<div>
							<!-- Section Header -->
							<div class="mb-4 flex items-end justify-between border-b border-white/10 pb-2.5 sm:mb-5 sm:pb-3">
								<h2 class="font-display text-xl font-bold tracking-tight text-white sm:text-2xl">
									Cardboard &amp; Beyond
								</h2>
							...
							{#if latestEpisode}
								...
								<a
									...
									target="_blank"
									rel="noopener noreferrer"
									class="group block min-w-0 overflow-hidden rounded-lg sm:rounded-xl border border-white/5 bg-gray-900/40 transition-all hover:border-white/15 hover:bg-gray-900/60"
								>
									<div class="sm:flex">
										<!-- Thumbnail -->
										<div class="relative aspect-video overflow-hidden bg-gray-800 sm:w-64 sm:shrink-0 md:w-72 lg:w-80">
										...
										<div class="flex min-w-0 flex-1 flex-col justify-center p-3.5 sm:p-5">
											<div class="mb-1.5 flex flex-wrap items-center gap-2 sm:mb-2">
												{#if latestEpisode.season && latestEpisode.episode}
													<span class="rounded bg-orange-500/15 px-2 py-0.5 text-[10px] sm:text-xs font-semibold text-orange-400">
														S{latestEpisode.season}E{latestEpisode.episode}
													</span>
												{/if}
												<span class="text-[10px] sm:text-xs font-semibold tracking-wide text-orange-400 uppercase">
													{currentPodcast.name}
												</span>
											</div>
											<h3 class="mb-1.5 text-base font-bold leading-tight text-white transition-colors group-hover:text-orange-400 sm:mb-2 sm:text-lg md:text-xl line-clamp-2">
												{latestEpisode.title}
											</h3>
							...
							{:else}
								<!-- No episodes yet -->
								<div class="overflow-hidden rounded-lg sm:rounded-xl border border-white/5 bg-gray-900/40">
									<div class="flex flex-col items-center gap-4 p-5 text-center sm:flex-row sm:items-center sm:gap-6 sm:p-6 sm:text-left">
										<img
											src={currentPodcast.coverImage || '/c&b.png'}
										...
										<div>
											...
											{#if currentPodcast.description}
												<p class="mt-2 text-xs text-gray-400 sm:text-sm">{currentPodcast.description}</p>
											{/if}
					...
					{:else}
						...
						<div>
							...
							<div class="overflow-hidden rounded-lg sm:rounded-xl border border-white/5 bg-gray-900/40">
								<div class="flex flex-col items-center gap-4 p-5 text-center sm:flex-row sm:items-center sm:gap-5 sm:p-6 sm:text-left">
									<img
										src="/c&b.png"
					...
					{#if remainingEpisodes.length > 0}
						<div>
							<div class="mb-3 flex items-end justify-between border-b border-white/10 pb-2.5 sm:mb-4 sm:pb-3">
								...
								{:else}
									<span class="text-[10px] text-gray-500 sm:text-xs">{currentPodcast?.episodes?.length || 0} total</span>
								{/if}
							</div>
								...
								{#each visibleEpisodes as episode (episode.id)}
									...
									>
										...
										{#if episode.thumbnailUrl}
											...
											/>
										{:else}
											<div class="flex h-12 w-20 shrink-0 items-center justify-center rounded-md bg-orange-500/10 sm:h-14 sm:w-24 sm:rounded-lg">
												<svg class="h-5 w-5 text-orange-400/60 sm:h-6 sm:w-6" fill="currentColor" viewBox="0 0 24 24">
													<path d="M8 5v14l11-7z" />
												</svg>
											</div>
										{/if}
										...
										<div class="min-w-0 flex-1">
											<div class="flex items-center gap-2">
												<h4 class="truncate text-xs font-semibold text-white group-hover:text-orange-400 sm:text-sm">{episode.title}</h4>
											</div>
											<div class="mt-0.5 flex flex-wrap items-center gap-1.5 text-[10px] text-gray-500 sm:mt-1 sm:gap-2 sm:text-xs">
												{#if episode.season && episode.episode}
													<span class="rounded bg-orange-500/10 px-1 py-0.5 text-orange-400/80 sm:px-1.5">S{episode.season}E{episode.episode}</span>
												{/if}
												{#if episode.guest}
					...
					<!-- Premium CTA (inline) -->
					{#if !isPremiumUser}
					<div class="relative isolate overflow-hidden rounded-xl border border-emerald-500/30 bg-gradient-to-br from-emerald-900/20 via-gray-900 to-purple-900/10 sm:rounded-2xl">
						<div class="pointer-events-none absolute top-0 right-0 h-48 w-48 rounded-full bg-emerald-500/10 blur-3xl sm:h-64 sm:w-64"></div>
						<div class="pointer-events-none absolute bottom-0 left-0 h-36 w-36 rounded-full bg-purple-500/10 blur-3xl sm:h-48 sm:w-48"></div>
						<div class="relative p-5 sm:p-6 md:p-8">
							<div class="mb-2 flex items-center gap-2 sm:mb-3">
								<svg class="h-4.5 w-4.5 text-emerald-400 sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 24 24">
									<path fill-rule="evenodd" d={icons.boltSolid} clip-rule="evenodd" />
								</svg>
								<h2 class="font-display text-lg font-bold text-white sm:text-xl">Unlock Everything with Premium</h2>
							</div>
							...
							<ul class="mb-4 space-y-1.5 sm:mb-6 sm:space-y-2">
								<li class="flex items-center gap-2 text-xs text-gray-300 sm:text-sm">
									<svg class="h-3.5 w-3.5 shrink-0 text-emerald-400 sm:h-4 sm:w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
				...
				<!-- Sidebar (1/3) -->
				<div class="min-w-0 flex flex-col gap-4 sm:gap-5 lg:gap-6">
					...
					{#if currentPodcast}
						<div class="rounded-lg border border-white/10 bg-gray-900/50 p-4 sm:rounded-xl sm:p-5">
							<div class="mb-3 flex items-center gap-2 sm:mb-4">
								<svg class="h-4.5 w-4.5 text-orange-400 sm:h-5 sm:w-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
							...
							{#if currentPodcast.youtubePlaylistUrl}
								...
								>
									<svg class="h-3 w-3 text-red-400 sm:h-3.5 sm:w-3.5" fill="currentColor" viewBox="0 0 24 24">
										<path d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0C.488 3.45.029 5.804 0 12c.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0C23.512 20.55 23.971 18.196 24 12c-.029-6.185-.484-8.549-4.385-8.816zM9 16V8l8 4-8 4z"/>
									</svg>
									Watch on YouTube
								</a>
					...
					{#if !isPremiumUser}
					<div class="rounded-lg border border-emerald-500/20 bg-gray-900/50 p-4 sm:rounded-xl sm:p-5">
						<div class="mb-2 flex items-center gap-2 sm:mb-3">
							<svg class="h-4.5 w-4.5 text-emerald-400 sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 24 24">
								<path fill-rule="evenodd" d={icons.boltSolid} clip-rule="evenodd" />
							</svg>
						...
						<ul class="mb-3 space-y-1.5 sm:mb-4 sm:space-y-2">
							<li class="flex items-center gap-2 text-[10px] text-gray-300 sm:text-xs">
								<svg class="h-3 w-3 shrink-0 text-emerald-400 sm:h-3.5 sm:w-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" d={icons.check} />
								</svg>
								Tournament VODs
							</li>
					...
					>
						<div class="flex items-center gap-3">
							<div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-800 sm:h-9 sm:w-9">
								<svg class="h-4 w-4 text-gray-400 transition-colors group-hover:text-white sm:h-4.5 sm:w-4.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
							...
							<svg class="h-3.5 w-3.5 text-gray-500 transition-colors group-hover:text-white sm:h-4 sm:w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
								<path stroke-linecap="round" stroke-linejoin="round" d={icons.arrowTopRightOnSquare} />
							</svg>
						</div>
					</a>
